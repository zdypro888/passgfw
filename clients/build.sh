...
generate_swift_config() {
    ...
    for i in $(seq 0 $((url_count - 1))); do
        local method=$(echo "$URLS" | python3 -c "import json,sys; print(json.load(sys.stdin)[$i]['method'])" 2>/dev/null || echo "api")
        local url=$(echo "$URLS" | python3 -c "import json,sys; print(json.load(sys.stdin)[$i]['url'])" 2>/dev/null || echo "")
        if [ -n "$url" ]; then
